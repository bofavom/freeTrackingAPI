...
import binanceParser from './../../../utility/binanceParser'
...
export default async (req, res, next) => {
  ...
  return res.status(422).json({error: 'Missing xlsx file.'})

  const xlsx = req.file.buffer
  const trades = binanceParser(xlsx)

  if (!trades || trades.length === 0)
  return res.status(404).json({
    error: 'No valid entries found in the xlsx file.'
  })
